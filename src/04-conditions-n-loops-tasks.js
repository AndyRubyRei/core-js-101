...
function doRectanglesOverlap(rect1, rect2) {
  ...
  const rect1IsToTheRight = rect1.left > rect2Right;
  const rect2IsToTheRight = rect2.left > rect1Right;
  const rect1IsBelow = rect1.top > rect2Bottom;
  const rect2IsBelow = rect2.top > rect1Bottom;

  // Check for overlap conditions
  if (rect1IsToTheRight || rect2IsToTheRight || rect1IsBelow || rect2IsBelow) {
...
function findFirstSingleChar(str) {
  ...
  return null; // No single character found
}
...
function getIntervalString(a, b, isStartIncluded, isEndIncluded) {
  ...
  return `(${start}, ${end})`; // No start or end included
}
...
function reverseInteger(num) {
  return +num.toString().split('').reverse().join(''); // Convert to string and then back to number
}
...
function toNaryString(num, n) {
  ...
  let result = '';
  while (initNum > 0) {
    result = (initNum % n).toString() + result;
...
function getCommonDirectoryPath(pathes) {
  ...
  for (let i = 0; i < pathComponents[0].length; i += 1) {
    const component = pathComponents[0][i];
    if (pathComponents.every((path) => path[i] === component)) {
      commonPath.push(component);
    } else {
      break;
    }
  }
...
function evaluateTicTacToePosition(position) {
  ...
  for (let i = 0; i < winningCombinations.length; i += 1) {
    ...
    if (
      ...
      return position[a[0]][a[1]]; // Winner found
    }
  }

  return undefined; // No winner (game is ongoing or a draw)
}
